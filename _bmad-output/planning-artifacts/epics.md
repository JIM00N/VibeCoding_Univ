...
- **FR-18 (직원 대리 예약)**: 직원이 예약 관리에서 환자를 골라 예약 폼(FR-6 P0 재사용)으로 대리 예약을 생성한다(walk-in 즉시진료 FR-16과 구분). (2026-07-25 correct-course)
...
> 경계(정직): 이 스토리는 예약 **생성**만 재사용한다. 예약 없이 즉시 진료·기록하는 walk-in(FR-16)과 (의사, 슬롯) 가용성 점유는 Epic 5.3(P1)이 담당한다. P0에서는 Epic 2와 동일하게 가용성 충돌 검사 없이 생성한다.
