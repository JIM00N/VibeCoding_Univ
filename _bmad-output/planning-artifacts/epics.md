...
- **FR-16 (P1, 2026-07-28 correct-course로 범위 축소)**: walk-in 환자 접수 — 직원이 진료과만 고르면 빈 의사가 자동 배정되고 가장 빠른 빈 시간으로 예약이 잡힌다. 빈 의사 없으면 거부(다른 시각 안내, 대기열 없음). **전용 경로(예약 없이 `appointment_id` null 기록)는 철회** — FR-18 대리 예약이 흡수한다(사유·트레이드오프: `sprint-change-proposal-2026-07-28.md`). 결과적으로 접수는 "다음 빈 슬롯"으로 잡히고(5.1 과거 시각 가드), `medical_record.appointment_id` nullable은 설계 여유로 남는다.
