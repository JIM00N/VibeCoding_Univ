...
임시계정 `demo01` ~ `demo40`, **비밀번호는 전부 `1234`**.
로그인 화면에 `demo01`~`demo08`이 버튼으로 깔려 있고, 나머지는 직접 입력합니다 (청중이 많을 때 각자 다른 계정을 쓰라고 40개를 뒀어요).
내 계정을 쓰고 싶으면 로그인 화면의 **회원가입**(`/signup`)에서 직접 만들 수 있습니다. 다만 **데모 시연은 임시계정으로 하세요** — 무대에서 폼을 채우는 시간이 아깝습니다.
비밀번호는 `/me/edit`에서 바꾸고, 잊었으면 `/forgot`에서 **아이디 + 가입 때 적은 이메일**로 되찾습니다(메일은 안 갑니다).
이메일은 `/me/edit`에서 언제든 등록·수정할 수 있어요 — 비어 있으면 마이페이지가 등록을 재촉합니다.
🔒 **임시계정 `demo01`~`demo40`은 비밀번호를 못 바꿉니다** — 한 명이 바꾸면 같은 계정을 쓰던 다른 청중이 잠기기 때문입니다. 발표 중에도 `1234`가 그대로 유지됩니다.
⚠️ 비밀번호는 평문으로 저장됩니다(D-4). 가입 화면에도 같은 경고가 있으니, 평소 쓰는 비밀번호는 넣지 마세요.
...
이미지 업로드 · **비밀번호 해싱** · **재설정 메일 발송·본인 인증**(아이디+이메일 대조로 대신한다) ·
**임시계정의 비밀번호 변경**(공유 계정이라 의도적으로 제외) ·
