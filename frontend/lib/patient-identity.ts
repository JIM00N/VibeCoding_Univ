...
// 환자 신원 컨텍스트 (FR-2, AD-8, UX-DR4). 선택한 환자를 localStorage 에 담아
// 화면 이동·새로고침에도 유지한다. 여기 저장된 정수 id 가 이후 환자용 조회의
// ?patient_id= 값이 된다(Epic 4). 앱 레벨 필터일 뿐 보안 격리가 아니다(AD-8).
//
// ⚠️ 왜 useSyncExternalStore 인가 (다른 방식은 이 레포에서 깨진다):
...
// 세션 한정 폴백. 이게 없으면 선택한 신원이 증발해 가드가 되돌리고 환자가 영영 못 들어간다.
...
// (a) 저장값이 없을 땐 무한 튕김, (b) 옛 값이 있을 땐 사용자가 고르지 않은 환자로 조용히 고정된다.
...
function parse(raw: string | null): PatientIdentity | null {
  ...
  try {
    ...
    // 이 id 는 Epic 4 에서 ?patient_id= 로 백엔드에 그대로 전달되므로 양의 정수여야 하고
